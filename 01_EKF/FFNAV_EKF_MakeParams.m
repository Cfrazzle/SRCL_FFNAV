...
%      x_k, y_k, z_k, theta_k, rt_k
%      x_dot_k, y_dot_k, z_dot_k, theta_dot_k, rt_dot_k
%
...
RAAN_target     = ;                    %Right-ascension of the Ascending Node (rad)
...
e_chaser        = e_target + 0.000005*10^1;              %Eccentricity - delta e
...
x0_chaser       = -0.075*0.5*10^1;               %Initial x-position of chaser(km)
...
%% Parameters for the Modified PRISMA Case

%Definitions for RWOP - Initial COE for Target Spacecraft
theta0_target   = 0;                    %True Anomaly (rad)
a_target        = 7500;                 %Semi-major Axis (km)
e_target        = 0.1;                  %Eccentricity
i_target        = 98.1877*pi/180;       %Inclination (rad)
RAAN_target     = 189.8914*pi/180;      %Right-ascension of the Ascending Node (rad)
AoP_target      = 1.0938*pi/180;        %Argument of Perigee (rad)
n              = sqrt(mu/a_target^3);   %Mean orbital motion of the target spacecraft
...
% Known Initial Conditions for In-Plane Elliptical Formation
x0_chaser       = -0.75*0.5;            %Initial x-position of chaser(km)
y0_chaser       = 0;                    %Initial y-position of chaser(km)
z0_chaser       = 0;                    %Initial z-position of chaser(km)
theta0          = theta0_target;        %Initial true anomaly of target (rad)
rt0             = rt0_target;           %Initial radius of target (km)
...
%% Paramters for the Proba-3 Case
%{
%===================================================
%Definitions for RWOP - Initial COE for Target Spacecraft
theta0_target   = 0;                    %True Anomaly (rad)
a_target        = 36943;%7500;                 %Semi-major Axis (km)
e_target        = 0.8111;%0;                    %Eccentricity
i_target        = 59*pi/180;%pi/6;                 %Inclination (rad)
RAAN_target     = 84*pi/180;%0;                    %Right-ascension of the Ascending Node (rad)
AoP_target      = 188*pi/180;%0;                    %Argument of Perigee (rad)
n               = sqrt(mu/a_target^3);  %Mean orbital motion of the target spacecraft

%Definitions for RWOP - Initial COE for Chaser Spacecraft
theta0_chaser   = theta0_target;        %True Anomaly (rad)
a_chaser        = a_target;             %Semi-major Axis (km)
e_chaser        = e_target + 0.000005*10^4;              %Eccentricity - delta e
i_chaser        = i_target;             %Inclination (rad)
RAAN_chaser     = RAAN_target;          %Right-ascension of the Ascending Node (rad)
AoP_chaser      = AoP_target;           %Argument of Perigee (rad)
...
% Known Initial Conditions for In-Plane Elliptical Formation
x0_chaser       = -1847.151409608149;%-0.075*0.5*10^4;               %Initial x-position of chaser(km)
y0_chaser       = 0;                   %Initial y-position of chaser(km)
z0_chaser       = 0;                    %Initial z-position of chaser(km)
theta0          = theta0_target;        %Initial true anomaly of target (rad)
rt0             = rt0_target;           %Initial radius of target (km)
...
GPS_r_STD = (1.2*10^-3)*1; %(km) - (-7) Equivalent of 1.2 m
GPS_v_STD = (0.03*10^-3)*1; %(km/s) - (-8) Equivalent of 0.03 m/s
...
%Initial Q Matrix (Covariance of the process noise, 10x10)
    sig2_r           = 2*10^-6; %Changing to -5 allows for 60000s simulation
    sig2_theta       = 5*10^-9;
    sig2_rt          = 5*10^-10; 
    sig2_v           = 5*10^-9; 
    ...
    Q0              = 10^0*[ sig2_r                           0 0 0 0 0 0 0 0 0 
